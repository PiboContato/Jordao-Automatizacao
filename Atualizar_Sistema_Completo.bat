...
echo [0/4] Compilando painel React (Front-end)...
cd render\frontend
call npm run build
cd ..\..

echo.
echo [1/4] Adicionando e criando commit local...
git add -A
git add -f render/frontend/dist/
git commit -m "%MSG%"
if %ERRORLEVEL% NEQ 0 (
...
echo.
echo [2/4] Enviando codigo para o GitHub (branch master)...
...
echo.
echo [3/4 - RENDER] Disparando sinal de deploy imediato no Render...
...
echo.
echo [4/4 - VM ORACLE] Atualizando e reiniciando o robo na VM Oracle via SSH...
...
echo ========================================================
echo  * Render (Dashboard Web): https://jordao-dashboard.onrender.com/
