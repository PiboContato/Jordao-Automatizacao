...
def obter_contexto_pagina(page: Page):
    try:
        if page.locator("text='RELATÓRIO DE DESPESAS'").is_visible(timeout=2000):
            return page
    except:
        pass
    try:
        iframe = page.frame_locator("iframe").first
        if iframe.locator("text='RELATÓRIO DE DESPESAS'").is_visible(timeout=2000):
            return iframe
    except:
        pass
    return page
...
def navegar(page: Page) -> None:
    logger.info("Navegando para Relatório de Conferência de Despesas via URL direta")
    fechar_popup_imoalert(page)
    
    try:
        page.goto(montar_url("rel-despesa"), timeout=15000)
...
def exportar_pdf(page: Page, data_inicio: str, data_fim: str = None) -> Path | None:
    logger.info("Iniciando geração e exportação (Relatório 11)")
    contexto = obter_contexto_pagina(page)

    try:
        # Pega a referência correta para o botão
        ...
        caminho_temp = Path(PASTA_DOWNLOADS_SO) / "relatorio_temporario_11.pdf"
        with open(caminho_temp, "wb") as f:
            f.write(pdf_bytes)
            
        logger.info("Sucesso Total! Arquivo PDF interceptado e salvo.")
        
        from src.utilitarios.conversor_conferencia_despesas import converter_para_excel
